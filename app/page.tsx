...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      {/* ৬. এক্সক্লুসিভ কালেকশন — ছবির কোলাজ + টাইটেল/বাটন (Settings থেকে নিয়ন্ত্রিত, প্রথম ছবি না দেওয়া পর্যন্ত হাইড থাকবে) */}
      {settings.exclusiveImage1 && (
        <section className="py-16 sm:py-20 bg-[#070706] border-t border-[#c9a054]/10">
          <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-10 items-center">
            <div className="grid grid-cols-2 gap-3">
              <div className="relative h-64 sm:h-80 col-span-1 row-span-2 rounded-2xl overflow-hidden border border-[#c9a054]/20">
                <img src={settings.exclusiveImage1} alt={settings.exclusiveTitle} className="w-full h-full object-cover" />
              </div>
              {settings.exclusiveImage2 && (
                <div className="relative h-32 sm:h-[9.5rem] rounded-2xl overflow-hidden border border-[#c9a054]/20">
                  <img src={settings.exclusiveImage2} alt="" className="w-full h-full object-cover" />
                </div>
              )}
              {settings.exclusiveImage3 && (
                <div className="relative h-32 sm:h-[9.5rem] rounded-2xl overflow-hidden border border-[#c9a054]/20">
                  <img src={settings.exclusiveImage3} alt="" className="w-full h-full object-cover" />
                </div>
              )}
            </div>
            <div className="text-center md:text-left space-y-4">
              <span className="text-[#c9a054] font-bold text-xs uppercase tracking-widest block">{settings.exclusiveSubtitle}</span>
              <h2 className="font-serif text-3xl sm:text-4xl font-bold text-white">{settings.exclusiveTitle}</h2>
              <Link
                href={settings.exclusiveButtonLink || "/"}
                className="inline-block bg-gradient-to-r from-amber-400 to-amber-600 text-black text-xs font-bold uppercase tracking-wider px-6 py-3 rounded-xl hover:shadow-lg transition-all"
              >
                {settings.exclusiveButtonText}
              </Link>
            </div>
          </div>
        </section>
