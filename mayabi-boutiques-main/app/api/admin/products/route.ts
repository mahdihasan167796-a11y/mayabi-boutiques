...
export async function GET(request: Request) {
  ...
  if (category) {
    ...
    // ডাটাবেসে category_slug বা category কলামের সাথে মেলানোর জন্য
    query = query.or(`category_slug.ilike.%${decodedCategory}%,category_slug.ilike.%${formattedCategory}%,category.ilike.%${decodedCategory}%,category.ilike.%${formattedCategory}%`);
  }
...
export async function POST(request: Request) {
  ...
  if (uploadError) {
    return NextResponse.json(
      {
        ok: false,
        error: `ছবি আপলোড ব্যর্থ হয়েছে: ${uploadError.message}. Supabase Storage-এ "${BUCKET}" নামে একটি পাবলিক বাকেট আছে কিনা যাচাই করুন।`,
      },
      { status: 500 }
    );
  }
